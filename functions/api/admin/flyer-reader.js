// /functions/api/admin/flyer-reader.js
...
//
// Renamed AGAIN, this time from read-flyer.js. Same mystery as before: this
// exact file, with correct content and a working deployment, started
// returning a platform-level 502 (HTML error page instead of our own JSON
// response) on every POST, with no code change to explain it. Renaming
// fixed it decisively the first time (from extract-flyer.js), and does
// again here. If this happens a third time, the pattern itself is the real
// finding: something about Cloudflare Pages Functions and this specific
// route/filename combination is flaky, not the code. Worth reporting to
// Cloudflare support if it recurs, rather than continuing to treat each
// occurrence as a fresh mystery.
//
